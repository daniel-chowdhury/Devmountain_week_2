...
// CODE HERE
const evenNumbers = mixedNumbers.filter(value => value%2 === 0)// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
// CODE HERE
const postTaxPrices  = prices.map(number => number*1.07)// = prices.map(/* Provide Your Callback Here );
...
// CODE HERE
const totalPopulation = populations.reduce((sum, element) => sum + element) //  = populations.reduce(/* Provide Your Callback Here */)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(monster_object => monster_object['CP'] > 200) // = 
...
// CODE HERE
const totals = orders.map((object) => object.price * (1 + object.tax))
console.log(totals)
...
// CODE HERE
const bobsTotal = purchases.filter(obj => obj.owner === "Bob").map(obj => obj.price).reduce((prev, curr) => prev + curr)

console.log("Bobs total is " + bobsTotal)